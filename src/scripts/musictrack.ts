...
export async function parseMusic(source: CacheFileSource, major: number, id: number, firstchunk: Buffer | null, allowdownload = false) {
    ...
    for (let [chunkindex, chunk] of chunkdatas.entries()) {
        ...
        for (let scan = 0; scan < chunk.length;) {
            ...
            let header = chunk.readUint32BE(scan); scan += 4;
            let version = chunk.readUint8(scan); scan += 1;
            let type = chunk.readUint8(scan); scan += 1;
            let granule = chunk.readBigInt64LE(scan); scan += 8;
            let series = chunk.readUint32LE(scan); scan += 4;
            let counter = chunk.readUint32LE(scan); scan += 4;
            let crc = chunk.readUint32LE(scan); scan += 4;
            let page_segments = chunk.readUint8(scan); scan += 1;
            let datalen = 0;
            for (let i = 0; i < page_segments; i++) {
                datalen += chunk.readUint8(scan); scan += 1;
            }
            ...
            let datachunk = Buffer.from(chunk.subarray(scan, scan + datalen));
            scan += datalen;
...
//crc implementation based on https://github.com/SteveLillis/.NET-Ogg-Vorbis-Encoder/blob/21f0770b5b93e16837c1b6dca3009096aff16231/OggVorbisEncoder/OggStream.cs
const oggtable = new Uint32Array([
...
export function crc32ogg(buf: Uint8Array | Uint8ClampedArray, crc = 0, rangeStart = 0, rangeEnd = buf.length) {
    crc = crc ^ 0;
    for (let i = rangeStart; i < rangeEnd; i++) {
        crc = (crc << 8) ^ oggtable[((crc >>> 24) & 0xff) ^ buf[i]];
    }
    return (crc ^ 0) >>> 0;
}